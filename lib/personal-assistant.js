...
(function() {
  ...
  PAFactory = function(options) {
    var PAConstructor, PersonalAssistant, globalMiddleware, handleCleanup, handleUpdates, heartbeatInterval, maxChunkSize, pas, pusher, redisClient, redisPub, redisSub, rootPa;
    globalMiddleware = [];
    pas = {};
    rootPa = null;
    heartbeatInterval = options.heartbeatInterval || 60 * 1000;
    redisPub = options.redisPub;
    redisSub = options.redisSub;
    redisClient = options.redisClient;
    handleUpdates = options.handleUpdates || false;
    handleCleanup = options.handleCleanup || false;
    maxChunkSize = options.maxChunkSize || MAX_CHUNK_SIZE;
    if (handleUpdates || handleCleanup) {
    ...
    PersonalAssistant = (function() {
      ...
      function PersonalAssistant(namespace) {
        ...
        */
        ...
        this.allQryIdsKey = "pa:qryIds";
        if (namespace) {
          this.allQryIdsKey += ":" + namespace;
        }
      ...
      */
      ...
      PersonalAssistant.prototype.runQuery = function(qry, callback) {
        ...
        qrypromise = qrypromise.then(function(existing) {
          ...
          existingQry = existing.result || null;
          if (existing.qryName) {
          ...
          return promise.then(function(qryResult) {
            if (qryResult) {
              ...
              promise = promise.then(function(_version) {
                ...
                promise = Q.ninvoke(redisClient, 'hmset', _this.qryIdKeyPref + qry.qryId, qryData);
                promise = promise.then(function() {
